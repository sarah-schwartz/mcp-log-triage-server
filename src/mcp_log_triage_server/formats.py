...
class LogParser(Protocol):
    ...
    def parse(self, line_no: int, line: str) -> Optional[LogEntry]:
        ...
...
def default_scan_config() -> ScanConfig:
    ...
    return ScanConfig(
        tokens={
            LogLevel.CRITICAL: (
                b"[CRITICAL]", b" CRITICAL ", b"[FATAL]", b" FATAL ", b" SEVERE ",
                b" PANIC ", b" EMERG ",
            ),
            LogLevel.ERROR: (
                b"[ERROR]", b" ERROR ", b' "level":"error"', b" level=error",
                b" EXCEPTION", b" TRACEBACK", b" FAILED", b" FAILURE",
            ),
            LogLevel.WARNING: (
                b"[WARNING]", b" WARNING ", b" WARN ", b' "level":"warning"', b" level=warning",
                b" DEPRECATED", b" RETRY", b" TIMEOUT",
            ),
            LogLevel.INFO: (
                b"[INFO]", b" INFO ", b' "level":"info"', b" level=info",
            ),
            LogLevel.DEBUG: (
                b"[DEBUG]", b" DEBUG ", b' "level":"debug"', b" level=debug", b" TRACE ",
            ),
        },
...
class LooseLevelParser:
    ...
    keywords: Optional[Dict[LogLevel, Iterable[str]]] = None
...
class SyslogParser:
    ...
    def _parse_rfc3164_ts(ts_str: str) -> Optional[datetime]:
        ...
        ts = naive.replace(year=now.year, tzinfo=timezone.utc)
    ...
    def _syslog_meta(*, pri: int, host: str, app: str) -> dict:
        sev = pri % 8
        fac = pri // 8
        return {"syslog": {"pri": pri, "severity": sev, "facility": fac, "host": host, "app": app}}
...
class AccessLogParser:
    ...
    def _parse_ts(ts_str: str) -> Optional[datetime]:
        try:
            return datetime.strptime(ts_str, "%d/%b/%Y:%H:%M:%S %z").astimezone(timezone.utc)
        except ValueError:
            return None
